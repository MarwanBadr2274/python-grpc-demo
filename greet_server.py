...
class GreeterService(greet_pb2_grpc.GreeterServicer):
    ...
    def ParrotSaysHello(self, request, context):
        ...
        for i in range(3):
            ...
            # use yield instead of return for streaming
            yield hello_reply
            time.sleep(3)
    ...
    def ChattyClientSaysHello(self, request_iterator, context):
        ...
        for request in request_iterator:
            print("ChattyClientHello Request Made:")
            print(request)
...
def serve():
    # define server and specify number of threads
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    # add the defined service to the server so that the server knows where to direct the request
